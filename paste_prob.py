...
def augment_images_with_labels(images_folder, labels_folder, masks_folder, output_folder, augmentation_prob, replace_existing=False):
    """
    ...
    for image_file in image_files:
        ...
        # with open(new_label_path, "a" if replace_existing else "w") as label_file:
        #     num_objects = random.randint(5, 10)
        #     sampled_masks = random.sample(mask_files, min(num_objects, len(mask_files)))
        ...
        with open(new_label_path, "a" if replace_existing else "a") as label_file: #change from w to a
            num_objects = random.randint(5, 10)
            sampled_masks = random.sample(mask_files, min(num_objects, len(mask_files)))
            ...
            for mask_file in sampled_masks:
                ...
                if mask is None or mask.shape[2] != 4:
                    print(f"Invalid mask: {mask_path}, skipping.")
                    continue
                ...
                if new_bbox is None:
                    # print(f"Failed to place mask: {mask_path}, skipping.")
                    continue
                ...
                class_id = 0
                label_file.write(convert_to_yolo_format(new_bbox, image_width, image_height, class_id) + "\n")
...
images_folder = "bg_yolo/images"
labels_folder = "bg_yolo/labels"
masks_folder = "sey_beri_v3/masks/passed_aug"

for augmentation_prob in probs:
    # output_folder = f"sey_beri_v3/sey_beri_v3_augmask_{int(augmentation_prob * 100)}prob_rp{replace_existing}"
    output_folder = f"bg_yolo/bg_yolo_augmask_{int(augmentation_prob * 100)}prob_rp{replace_existing}"
    augment_images_with_labels(images_folder, labels_folder, masks_folder, output_folder, augmentation_prob, replace_existing)
